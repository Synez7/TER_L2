...
def jeu(etat):
    ...
    while not fin_partie and len(possibles(etat)) != 0:  
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:  ## Evenement : clic gauche de la souris sur un emplacement inoccupée de la grille de jeu
                ...
                if tour == humain:
                    ...
                    if emplacementValide(etat, col):
                        ...
                        if estGagnant(etat,4,"H",[0,0]):     ## Si l'utilisateur remporte la partie, un message s'affiche en haut de la grille
            ...
            if(possibles(etat) == [] and not (estGagnant(etat, 4, "IA",[0,0]) == True) and not (estGagnant(etat, 4, "H",[0,0]) == True)):
